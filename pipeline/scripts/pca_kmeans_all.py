...
#get data
file_list = glob.glob('stats_all/*.stats_all')
...
params = sys.argv[1:]
if not header_col :
    header_col = (1,4,5) #default is rank, dir, occ
    params = 'rank,dir,occ'
...
#print("final data ", normed_data, "\n",  normed_data.shape)

res_pca = pca.fit_transform(normed_data)

#don't try to do this it just doesn't work
#res_pca = pca.fit(normed_data)
...
if True :
    ...
    fig.suptitle('K-means [all] - features : '+str(params), fontsize=16)
...
fig.suptitle('PC Explained Variance [all] - features : '+str(params), fontsize=16)
fig.savefig('results/explained_var.png')
...
fig.suptitle('PCA [all] - features : '+str(params), fontsize=16)

fig.savefig('results/pca_1_2_3_4.png')
