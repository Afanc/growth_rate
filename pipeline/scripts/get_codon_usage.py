...
def get_codon_usage(data, file_list) :
    ...
    for num,i in enumerate(data) :
        ...
        #image = np.zeros(shape=(648, 64))
        image = np.ones(shape=(648, 64))*127 #neutral is gray
        ...
        for pos,k in enumerate(gene_positions) :
            ...
            #count em
            temp_dic = CodonsDict.copy()

            for codon in codons :
                c = codon.upper()
                try: 
                    temp_dic[c] += direction #+=1 or -= 1
                except:
                    pass

            #in an image, order is same as initial dict
            image[pos,:] = np.fromiter(temp_dic.values(), dtype=float)

        image[:number_genes,...] += 127
        ...
        if False:
            ...
            robust = RobustScaler()
            #normalized_image = standardscaler.fit_transform(image[:number_genes,...].astype(np.uint8))
            normalized_image = robust.fit_transform(image[:number_genes,...].astype(np.uint8))
            normalized_image *= 127.0/normalized_image.max()
            normalized_image += 127
            ...
            [minmax_image, tmm_image, equ_image, clahe_image, normalized_image] = [np.concatenate((i, image[number_genes:,...].astype(np.uint8)), axis=0) for i in [minmax_image, tmm_image, equ_image, clahe_image, normalized_image]]

            sns.set_style('darkgrid')

            #all_titles = ['raw', 'minmax', 'tmm', 'equ hist', 'clahe']
            #all_images = [image, minmax_image, tmm_image, equ_image, clahe_image]
            all_titles = ['raw', 'minmax', 'robust', 'clahe']
            all_images = [image, minmax_image, normalized_image, clahe_image]
            all_colors = ['black', 'red', 'green', 'blue']
            ...
            fig.savefig('images/comp_'+file_list[num][6:-5]+'jpg', bbox_inches='tight')
            ...
            fig.savefig('images/hist_'+file_list[num][6:-5]+'jpg', bbox_inches='tight')
        ...
        final_image.save('images/'+file_list[num][5:-5]+'png')
        print("done !")
